...
通过观察 http 的请求过程可以看到，HTTP 的工作模式是非常简单的，由于 TCP/IP 协议负责底层的具体传输工作，HTTP 协议基本上不用管底层传输。单从这一点上来看，所谓的“超文本传输协议”其实并不怎么管“传输”的事情。

那么 HTTP 协议的核心部分是什么呢？答案就是它传输的报文内容。
...
![](../images/12-message-structure_20210104113504.png)
...
![](../images/12-message-structure_20210104113721.png)

看一下我们之前用 Wireshark 抓的包吧。

![](../images/12-message-structure_20210104113820.png)

在这个浏览器发出的请求报文里，第一行“GET / HTTP/1.1”就是请求行，而后面的“Host”“Connection”等等都属于 header，报文的最后是一个空白行结束，没有 body。

在很多时候，特别是浏览器发送 GET 请求的时候都是这样，HTTP 报文经常是只有 header 而没 body，相当于只发了一个头过来

不过这个“头”也不能太大，虽然 HTTP 协议对 header 的大小没有做限制，但各个 Web 服务器都不允许过大的请求头，因为头部太大可能会占用大量的服务器资源，影响运行效率。
...
![](../images/12-message-structure_20210104142419.png)
...
别看请求行就一行，貌似很简单，其实这里面的“讲究”是非常多的，尤其是前面的请求方法和请求目标，组合起来变化多端，后面还会详细介绍。
...
![](../images/12-message-structure_20210104142843.png)
...
![](../images/12-message-structure_20210104143054.png)

![](../images/12-message-structure_20210104143107.png)
...
![](../images/12-message-structure_20210104145901.png)
...
3. 欢迎你把自己的答案写在留言区，与我和其他同学一起讨论。如果你觉得有所收获，也欢迎把文章分享给你的朋友。
